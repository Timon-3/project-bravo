...
qr_code_text="Let it snow!"

start_time=6

font5 = ImageFont.truetype(os.path.join(picdir, 'Font.ttc'), 10)
...
try:
    ...
    # Add Room-State
    draw.text((180, 0), ROOM_STATE[1], font = font34, fill = 0)
    ...
    draw.text((10, 35), 'Kalender', font = font24)
    ...
    for i in range (calendar_rows):
        ...
        # Write time text
        if i>0:
            draw.text((calendar_pos[0]+calendar_size[0]/calender_col/4, calendar_pos[1]+calendar_size[1]/calendar_rows*i), str(i+start_time-1)+":00", font = cal_font, align="center")
        if i+start_time == 13:
    ...
    draw.text((232, 290), "Zuletzt Aktualisiert:"+ str(dt.strftime('%d.%m.%Y, %H:%M:%S')), font = font5)
    draw.rectangle((200, 200, 60, 250))
